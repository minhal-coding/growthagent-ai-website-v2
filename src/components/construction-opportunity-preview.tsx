...
export function ConstructionOpportunityPreview({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section
      ...
      className={`ga-command-shell overflow-hidden border border-white/15 bg-[#070a10]/95 shadow-[0_38px_120px_rgba(0,0,0,0.56)] ${compact ? "rounded-xl" : "rounded-[1.4rem]"}`}
    >
      <div className={`flex flex-wrap items-center justify-between border-b border-white/10 bg-[#0a0e16] ${compact ? "min-h-11 gap-2 px-3 py-2" : "min-h-14 gap-3 px-4 py-3 sm:px-5"}`}>
      ...
      <div className={`border-b border-[#e4bd45]/25 bg-[linear-gradient(90deg,rgba(228,189,69,0.12),rgba(228,189,69,0.025))] font-semibold text-slate-200 ${compact ? "px-3 py-1.5 text-[0.64rem] leading-4" : "px-4 py-2.5 text-xs leading-5 sm:px-5"}`}>
        {compact
          ? "Demonstration data — every record, source, jurisdiction, and status is fictional."
      ...
      <div className={`grid ${compact ? "md:grid-cols-[3rem_minmax(0,1fr)]" : "md:grid-cols-[3.75rem_minmax(0,1fr)]"}`}>
        <aside aria-label="Demonstration navigation" className={`hidden border-r border-white/10 bg-[#06090e] md:flex md:flex-col md:items-center md:justify-between ${compact ? "py-2.5" : "py-4"}`}>
          <div className={compact ? "space-y-1" : "space-y-2"}>
            {railItems.map((item) => (
              <span
                key={item.label}
                title={item.label}
                className={`grid place-items-center border ${compact ? "size-8 rounded-lg" : "size-10 rounded-xl"} ${item.active ? "border-[#42d6c7]/50 bg-[#42d6c7]/10 text-[#72eee2]" : "border-transparent text-slate-500"}`}
              >
          ...
          <span className={`grid place-items-center border border-white/10 text-slate-500 ${compact ? "size-8 rounded-lg" : "size-10 rounded-xl"}`} title="Notifications">
            <Bell className="size-4" aria-hidden="true" />
            <span className="sr-only">Notifications</span>
          </span>
        </aside>

        <div className={compact ? "grid lg:grid-cols-[0.78fr_1.22fr]" : "grid lg:grid-cols-[0.76fr_1.24fr]"}>
          <div className={`border-b border-white/10 bg-[#080c13] lg:border-b-0 lg:border-r ${compact ? "p-2.5" : "p-3 sm:p-4"}`}>
            <div className={`flex items-center gap-2 border border-white/10 bg-black/25 text-slate-400 ${compact ? "mb-2 rounded-lg px-2 py-1.5 text-[0.64rem]" : "mb-3 rounded-xl px-3 py-2 text-xs"}`}>
            ...
            <div className={compact ? "space-y-1.5" : "space-y-2"} role="list" aria-label="Demonstration opportunity records">
              {records.map((record) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    className={`w-full border text-left transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#42d6c7] ${compact ? "rounded-lg p-2" : "rounded-xl p-3"} ${
                      active
                  ...
                  >
                    <span className="flex items-start justify-between gap-3">
                      <span className="min-w-0">
                        ...
                        <span className={`block font-bold text-white ${compact ? "mt-0.5 text-[0.68rem] leading-4" : "mt-1 text-xs leading-5 sm:text-sm"}`}>{record.title}</span>
                        <span className={`block truncate text-slate-500 ${compact ? "mt-0.5 text-[0.62rem]" : "mt-1 text-xs"}`}>{record.area}</span>
                      </span>
          ...
          <div className={`relative overflow-hidden bg-[radial-gradient(circle_at_88%_8%,rgba(66,214,199,0.08),transparent_32%)] ${compact ? "p-3" : "p-4 sm:p-5"}`}>
            ...
            <div className="relative">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <h3 className={`max-w-xl font-extrabold tracking-[-0.035em] text-white ${compact ? "mt-1 text-base leading-5" : "mt-1.5 text-xl sm:text-2xl"}`}>{selected.title}</h3>
                </div>
              ...
              <div className={`grid gap-2 sm:grid-cols-2 ${compact ? "mt-2" : "mt-4"}`}>
                <InfoRow compact={compact} icon={MapPin} label="Service area" value={selected.area} />
                <InfoRow compact={compact} icon={Building2} label="Source type" value={selected.source} />
              </div>

              <div className={`grid gap-3 border-y border-white/10 sm:grid-cols-[1fr_auto] sm:items-center ${compact ? "mt-2 py-2" : "mt-4 py-4"}`}>
                <div>
                  ...
                  <div className={`${compact ? "mt-1.5" : "mt-2"} flex flex-wrap gap-1.5`}>
                    {["Evidence slot", selected.fit, "Human decision"].map((item, index) => (
                      <span key={item} className={`inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-black/20 font-semibold text-slate-300 ${compact ? "px-2 py-0.5 text-[0.62rem]" : "px-2.5 py-1 text-[0.68rem]"}`}>
                ...
                <div className={`flex items-center rounded-xl border border-[#42d6c7]/20 bg-[#42d6c7]/[0.04] ${compact ? "gap-2 px-2.5 py-1.5" : "gap-3 px-3 py-2.5"}`}>
                  <span className={`grid place-items-center rounded-full border border-[#42d6c7]/30 text-[#42d6c7] ${compact ? "size-7" : "size-8"}`}>
              ...
              <p className={`text-slate-400 ${compact ? "mt-2 text-[0.68rem] leading-4" : "mt-3 text-xs leading-5"}`}>{selected.note}</p>
              <div className={`flex items-center justify-between gap-4 rounded-xl border border-white/10 bg-[#05070d]/94 ${compact ? "mt-2 p-2" : "mt-3 p-3"}`}>
...
function InfoRow({ compact = false, icon: Icon, label, value }: { compact?: boolean; icon: typeof MapPin; label: string; value: string }) {
  return (
    <div className={`flex rounded-xl border border-white/10 bg-black/20 ${compact ? "gap-2 p-2" : "gap-2.5 p-2.5"}`}>
      <Icon className="mt-0.5 size-3.5 shrink-0 text-[#f2c94c]" aria-hidden="true" />
      <div className="min-w-0">
        <p className="text-[0.65rem] text-slate-500">{label}</p>
        <p className={`mt-0.5 font-semibold text-slate-200 ${compact ? "text-[0.65rem] leading-4" : "text-xs leading-5"}`}>{value}</p>
      </div>
    </div>
  );
}
