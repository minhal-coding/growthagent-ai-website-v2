...
export function FloridaTrustSection({ compact = false }: { compact?: boolean }) {
  ...
  const visibleAreas = compact
    ? ["Orange County — illustrative", "Seminole County — illustrative", "Osceola County — illustrative", "Central Florida — illustrative"]
    : launchAreas;

  return (
    <section aria-labelledby="florida-trust-title" className={`relative overflow-hidden border-y border-white/10 bg-[#06090f] px-5 sm:px-8 lg:px-12 ${compact ? "py-10 sm:py-12" : "py-20 sm:py-28"}`}>
      <div className="pointer-events-none absolute inset-0 ga-blueprint opacity-25" aria-hidden="true" />
      <div className="relative mx-auto max-w-[90rem]">
        <div className="mx-auto max-w-4xl text-center">
          {!compact ? <p className="text-xs font-extrabold uppercase tracking-[0.24em] text-[#e4bd45]">Florida launch research</p> : null}
          <h2 id="florida-trust-title" className={`${compact ? "mt-1" : "mt-5"} text-balance font-extrabold leading-[1.02] tracking-[-0.05em] text-white ${compact ? "text-4xl sm:text-[2.65rem] lg:text-[2.9rem]" : "text-4xl sm:text-6xl lg:text-7xl"}`}>
            {compact ? <>Starting in Florida. <span className="bg-[linear-gradient(90deg,#f2c94c,#d99d2b)] bg-clip-text text-transparent">Built around evidence.</span></> : "A focused market. An explicit trust ledger."}
          </h2>
          <p className={`mx-auto max-w-3xl text-base leading-7 text-slate-300 sm:text-lg sm:leading-8 ${compact ? "mt-3" : "mt-6"}`}>
            {compact ? "The initial launch is focused on Florida construction teams and source-backed public-record workflows under validation." : "The planned first-market work combines explicit service-area validation with controls for evidence, contacts, scope, and human review."}
          </p>
          <p className={`mx-auto inline-flex max-w-3xl items-center gap-2 rounded-full border border-[#e4bd45]/40 bg-[#e4bd45]/[0.07] px-4 py-2 text-sm font-extrabold text-[#f2c94c] ${compact ? "mt-3" : "mt-6"}`}>
            <Radio className="size-4 shrink-0" aria-hidden="true" />
            Illustrative validation areas — not active coverage.
          </p>
        </div>

        <div className={`${compact ? "mt-6" : "mt-12"} overflow-hidden rounded-[1.6rem] border border-white/12 bg-[#070b12]/94 shadow-[0_38px_110px_rgba(0,0,0,0.36)]`}>
          <div className="grid lg:grid-cols-[0.92fr_1.08fr]">
            <div className={`relative overflow-hidden border-b border-white/10 p-5 lg:border-b-0 lg:border-r sm:p-8 ${compact ? "min-h-[24rem]" : "min-h-[34rem]"}`}>
              <div className="absolute left-5 top-5 z-10 sm:left-8 sm:top-8">
                ...
                <p className="mt-2 max-w-xs text-sm leading-6 text-slate-400">{compact ? "Counties shown are illustrative launch examples. Coverage is not active or statewide." : "Generic research areas show how geography could guide a future service-area review."}</p>
              </div>
            ...
            <div className="p-5 sm:p-8">
              <div className="flex items-end justify-between gap-5 border-b border-white/10 pb-5">
                <div>
                  ...
                  <h3 className="mt-2 text-3xl font-extrabold tracking-[-0.04em] text-white">{compact ? "Trust & Safety Ledger" : "Controls before claims."}</h3>
                </div>
              ...
              <div>
                {visiblePrinciples.map((item) => (
                  <article key={item.title} className={`grid gap-3 border-b border-white/10 sm:grid-cols-[auto_1fr] ${compact ? "py-4" : "py-5"}`}>
                    ...
                    <div>
                      ...
                      {!compact ? <p className="mt-2 text-sm leading-6 text-slate-400">{item.text}</p> : null}
                    </div>
                  </article>
                ))}
              </div>
