import { Check, Circle, ListX, ShieldCheck } from "lucide-react";
...
export function LeadListComparison() {
  return (
    <section aria-labelledby="comparison-title" className="ga-content-visibility relative overflow-hidden border-y border-white/10 bg-[#05080d] px-5 py-20 sm:px-8 sm:py-28 lg:px-12">
      ...
      <div className="relative mx-auto max-w-[90rem]">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <h2 id="comparison-title" className="mt-5 text-balance text-4xl font-extrabold tracking-[-0.05em] text-white sm:text-6xl">More than a lead list.</h2>
        ...
        <MotionStagger className="mt-12 grid gap-5 lg:grid-cols-2">
          <GlowCard className="h-full rounded-2xl border border-white/10 bg-[#080b11] p-6 sm:p-8">
            ...
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {ordinaryLeadList.map((item) => <li key={item} className="flex items-center gap-2.5 rounded-xl border border-white/[0.07] bg-black/20 px-3 py-3 text-sm text-slate-400"><Circle className="size-3.5 shrink-0 text-slate-600" aria-hidden="true" />{item}</li>)}
            </ul>
          </GlowCard>
